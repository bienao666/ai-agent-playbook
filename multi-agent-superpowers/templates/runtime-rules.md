...
1. Manager 定义目标、验收标准、验证方式。
2. Manager 输出 `管理决策`。
3. Builder 做最小必要变更。
4. Reviewer 对照验收标准检查。
5. Needs Changes 时回到 Builder。
6. Pass 后 Manager 汇总。
7. 记录 task log、iteration log、必要 decisions / lessons。
```
...
记录架构选择、高风险判断、Reviewer 分歧后的 Manager 仲裁、用户拍板方向。不记录琐碎实现。
```
...
记录测试失败原因、工具限制、Reviewer 重要发现、重复问题。保持简短可执行。
```
...
记录限制，使用本地降级方案，必要时更新 capabilities。
